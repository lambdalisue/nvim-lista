...
function! s:_new_undolist() abort
  ...
  let undolist.tempfile = tempname()
  execute 'silent wundo!' undolist.tempfile
  return undolist
endfunction
...
let s:winview = {}
function! s:_new_winview() abort
  let winview = copy(s:winview)
  let winview.saved = winsaveview()
  return winview
endfunction
function! s:winview.restore() abort
  call winrestview(self.saved)
endfunction

let s:input = {}
function! s:_new_input() abort
  let input = copy(s:input)
  call inputsave()
  return input
endfunction
function! s:input.restore() abort
  call inputrestore()
endfunction

let s:cursor = {}
function! s:_new_cursor() abort
  let cursor = copy(s:cursor)
  let cursor.saved = getpos('.')
  return cursor
endfunction
function! s:cursor.restore() abort
  call setpos('.', self.saved)
endfunction
...
function! s:store(targets) abort
  ...
  for meta in a:targets
    ...
    elseif type(meta) == type('')
      ...
      elseif meta ==# 'undolist'
        call add(resources, s:_new_undolist())
      elseif meta ==# 'winview'
        call add(resources, s:_new_winview())
      elseif meta ==# 'cursor'
        call add(resources, s:_new_cursor())
      elseif meta ==# 'input'
        call add(resources, s:_new_input())
      else
        call s:_throw(printf(
